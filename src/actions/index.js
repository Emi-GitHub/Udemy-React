...
export const createStream = formValues => async (dispatch, getState) => {
    ...
    dispatch({type: CREATE_STREAM, playload: response.data})
    //do some programmatic navigation to get the user back to the toot route
    history.push('/');
}

export const fetchStreams = () => async dispatch => {
    const response = await streams.get('/streams');
    dispatch({ type: FETCH_STREAMS, playload: response.data})
} 
